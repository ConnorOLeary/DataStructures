...
//  DataStructures
//
//  Created by Oleary, CONNOR on 3/19/18.
...
public:
    ...
    boll areConnected(int source, int target)const;
    std::set<int>neighbors(int vertex)const;
    
    void depthFirstTraversal(Graph<Type>& graph, int vertex);
    void breadthFirstTraversal(Graph<Type> & graph, int vertex);
    int constTraversal(Graph<Type> & graph, int vertex);
}
...
template <class Type>
void Grpah<Type> :: removeEdgeCost(int source, int target)
{
    assert(source >= 0 && source < vertexCount && target >= 0 && target < vertexCount);
...
template <class Type>
bool Graph<Type> :: hasUndirectedConnect(int source, int target) const
{
    assert(source >= 0 && source < vertexCount && target >= 0 && target < vertexCount);
...
{
    std::set<int> connections = currentGraph.neighbors(vertex);
    std::set<int>::iterator setIterator;
    
    visited[verted] = true;
    cout << currentGraph[vertex] << ", " << endl;
    
    for(setIterator = connections.begin(); setIteraror != connections.end(); setIterator++)
...
{
    assert(vertex < currentGraph.size());
    bool visisted[MAXIMUM];
    std::set<int> connections;
    std::set<int>::iterator setIterator;
    std::queue<int> vertexQueue;
    
    std::fill_n(visisted, currentGraph.size(), false);
    visited[vertex] = true;
    cout<< currentGraph[vertex] << endl;
...
int Graph<Type> :: costTraversal(Graph<Type> & currentGraph, int start)
{
    ...
    while(!vertexQueue.empty())
    {
        ...
        connections = currentGrpah.neighbors(currentIndex);
        vertexQueue.pop();
